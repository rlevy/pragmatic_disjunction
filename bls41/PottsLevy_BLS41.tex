...
\title{Negotiating Lexical Uncertainty and Expertise\\ with Disjunction}				%TITLE HERE
\author{\sc Christopher Potts and Roger Levy\\ 						%AUTHOR HERE
{\it Stanford University and UC San Diego}\footnote{For help with the data, we thank Arto Antilla, James Collins, Itamar Francez, Kumiko Fukumora, Justine Kao, and Natalia Roudakova.
Thanks also to Larry Horn and audiences at the 2015 LSA Annual Meeting and ESSLLI 2014. This research was supported in part by NSF IIS 1159679 and ONR N00014-10-1-0109.}} %AFFILIATION HERE
\date{}
...
against that possibility. This is a defensive position; the speaker's
...
\subsection{Anxious experts}\label{sec:full}
...
we propose to apply uncertainty only to the true lexical items, and
then close each lexicon under the compositional operations of
disjunction and conjunction. Thus, in \figref{fig:rsa-disj}, only $p$
and $q$ can be refined.  This results in nine distinct lexica.  Each
of these is then expanded to include $\pandq$ and $\porq$, with their
meanings determined by whatever meanings the atomic expressions have.
...
illustrate the effects using parameters chosen by hand, and then we
explore the space of parameters to try to determine which settings ---
which approximations of the context --- deliver each of the readings.
...
In our model, the importance of lexical communication is governed by
the parameter $\beta$. If this is set to $0$, then all lexical
preferences are silenced. Where $\beta$ is positive, the ratio of
$\alpha$ and $\beta$ is a rough guide to the relative importance of
...
how the model arises at definitional construals and intentions.  At
...
intention. It's slight, but it is enough to break the asymmetry
for $\ListenerK[2]$, which is strongly biased toward definitional
...
minimal variant of (the topmost table in) \figref{fig:def} in which
this extra lexical requirement has been imposed. This removes
$\LexStar$ from consideration. The listener inference is numerically
stronger, and it is paralleled by a similar increased probability on
the speaker side for producing \word{A or X} with definitional
intentions. We will explore these `focal' contexts in more detail in
...
In \secref{sec:data:overlapping}, we identified two kinds of
implicature inferences that are relevant for HG violations:
Q-implicatures, in which the speaker blocks a potential upper-bounding
implicature created by a general term, and I-implicatures, in which
the speaker blocks a potential inference from a general term to a
specific salient sub-kind. Our model of anxious experts is able to
derive both kinds of inferences.

\Figref{fig:qsims} reports on simulations involving the Q-implicture
...
\begin{figure}[tp]
  ...
  \caption{Q-implicature simulations. The x-axis variation is created
...
\begin{figure}[tp]
  ...
  \caption{I-implicature simulations. The x-axis variation is created
...
The above simulations show that the model can simiulate not only
implicature inferences but also the kind of behavior that would
normally be characterized as implicature blocking. This possiblity
...
examples. Since neither kind of implicature is a primative of the
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.9\textwidth]{fig/paramexplore-lex5}
...
\begin{figure}[tp]
  \centering
  \includegraphics[width=0.9\textwidth]{fig/paramexplore-lex5-focal}
