...
instance, a speaker who uses the phrase \word{cheap or free} might be
concerned that using \word{cheap} alone will trigger a scalar
implicature \citep{Hirschberg85} excluding zero cost. The HG violation
then serves to cancel or block this unwanted
inference. \citet{Chemla-HurfordCounts} studies this class of
...
compelled to say as little as possible, and the listener is advised to
seek out specific interpretations. As a corollary of this, general
...
\secref{sec:model} also supports this reasoning, but it requires no
...
HG-violating ones, so generalizing about their usage conditions is
difficult.  The examples in \eg{ted} were obtained by annotating the
disjunctive clauses in a sample of 98 TED talks. These talks seem
ideal for finding definitional uses, because the speakers are experts
with broadly pedagogical aims. In addition, videos of the talks are
available online, which made it possible for us to use intonation and
other cues to try to verify that the speakers had definitional
intentions. These seven examples were drawn from a set of 344
disjunctive clauses, for a rate of only about 2\%, despite the context
being conducive to such uses. The examples are given with the
...
as many distinctions as possible from its estimate of the listener, and
