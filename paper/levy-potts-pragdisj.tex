...
$\sem{\word{X\/}}-\sem{\word{Y\/}}$ for some \word{Y}, and the
disjunction is a hedge against that possibility. This is a defensive
...
options in the current discourse \citep{Simons:2001}. If HG were
adopted as an explicit theoretical constraint, then the possibility of
doing this would more or less follow --- we just require the
additional premise that the listener is charitable and so will try to
find an acceptable construal of the utterance. The model we develop in
...
This agent's best inferences are in gray. It has already learned to
separate the related terms: $\porq$ conveys that $\pandq$ is false,
and the atomic propositions convey biases not present in their
disjunction. One can also see that the speaker seeks to avoid
ambiguities. For instance, where an atomic proposition is true, the
speaker opts for it, thereby creating less uncertainty in the listener
than a disjunction would.
...
the extent to which the current message biases in favor of $\Lex$.
...
\begin{figure}[htp]
  ...
  \newcommand{\angledarrowdivider}{& \multicolumn{3}{c}{\swarrow \hspace{45pt} \downarrow\hspace{45pt} \searrow}}
...
intention, but it's slight, and it is enough to break the asymmetry
...
quantity this intuitive characterization in
...
\begin{figure}[tp]
  ...
  $\begin{array}[c]{l l r r r}
    ...
    \midrule
    \Lex_{1} & \smalldisjlex{w_{1}}{w_{2}}{w_{2}}  & 0 & 0 & 0 \\[1ex]
    \Lex_{2} & \smalldisjlexTargetDef             & \graycell{1} & 0 & 0\\
    \bottomrule
  \end{array}$
...
\Figref{fig:hurford} presents a context in which the $\ListenerK[2]$
