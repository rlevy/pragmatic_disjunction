...
proper entailment relation, it is generally possible to find a context
...
$\sem{\word{X}}-\sem{\word{Y}}$ for some \word{Y}, and the disjunction
is a hedge against that possibility. This is a defensive position; the
speaker's own lexicon might allow her to use just the general term to
convey her intentions, but she is concerned that the listener will
arrive at a different conclusion. The costs of disjunction are
therefore worth paying even if it adds no new information given the
...
a derive, pragmatically-enriched probabilistic interpretation
function:
%
\begin{examples}
\item\label{l1}% 
...
further recursion of these agents lexicon-specific agents, but we do
allow further recursion of the agents we define next.
...
on the model of \citet{Smith:Goodman:Frank:2013}. It reseasons in
terms of the space of lexica defined by \subeg{model-extend}{lexset}:
%
\begin{examples}
  \item\label{L1}%
...
\Figref{fig:modstruc} is a schematic diagram showing how our different
agents relate to each other. Intuitively, if one begins at, say,
...
up from the lexica and getting pooled together by $\ListenerK[1]$,
which then makes joint inferences about the state and the
lexicon. 
...
flat (treats all lexica identicall).
...
signaling, as in examples like that of \figref{fig:rsa-disj}. To
achieve this, one can sum over lexica, as in \eg{lisnorm} and
