...
\headerbox{Hurfordian perceptions and intentions}{name=hurford,column=0,row=2,span=2,below=intro}{
  ...
  \begin{minipage}[c]{0.48\linewidth}
    ...
    \exitem{1} & the nuptials will take place in either \highlight{France or Paris} \\
    \exitem{2} & the \highlight{canoe or boat} will be held by the stream's current \\
    \exitem{3} & In 1940, 37\% of us had gone to a \highlight{church or synagogue} in the last week.
    \end{tabular}
...
\headerbox{Disjunctive definition and identification}{name=def,column=0,row=2,span=2,below=hurford}{
  ...
  \begin{minipage}[c]{0.48\linewidth}
    \renewcommand{\arraystretch}{1.2}
    \begin{tabular}[c]{r@{ \ }p{0.85\textwidth}}
    \exitem{4} & wine lover or \emph{oenophile} \\
    \exitem{5} & A Geological History of Manhattan or New York Island \\
    \exitem{6} & New Haven or ``the Elm City''  \\
    \exitem{7} & woodchuck or ``land beaver''
    \end{tabular}    
  \end{minipage}
  \hfill 
  \begin{minipage}[c]{0.48\linewidth}
  ...
  \begin{tabular}[c]{@{}  r@{ }c c c  @{}} 
    \multicolumn{4}{c}{
      ...
      $\begin{array}[c]{l@{ }l r r r}
         ...
         \Lex_{2} & \smallhurfordlex{{\color{lexcolor}w_{1}}}{w_{2}}{{\color{lexcolor}w_{1}}} &  \graycell{.77} & 0 & .06 \\
         \bottomrule
       \end{array}$} \\
...
\headerbox{Hurfordian contexts}{name=hurfordmodel,column=3,row=1,span=1,below=model}{
  ...
  $\begin{array}[c]{l@{ }l r r r}
     ...
     \Lex_{1} & \smallhurfordlex{{\color{lexcolor}w_{1}}}{w_{2}}{{\color{lexcolor}w_{2}}} & .04 & 0 & \graycell{.45} \\[1ex]
